...
├── densest_subgraph_raw/    # 密集子图原始数据（当前为空）
└── README.md
```
...
- `densest_subgraph_raw/` 当前为空，后续若有独立的密集子图测试数据再另行下载
...
`maximum_clique_raw/` 和 `densest_subgraph_raw/` 存放从网络下载的原始格式数据（.mtx、.graph 等）。这些文件**不直接用于实验**，需先通过 `converters/` 下的脚本转为统一 JSON。
